...
class OZ_PdaPageContacts : OZ_PdaPage
{
    ...
    // Ключ обраного -- для дій, яким потрібна АДРЕСА, а не підпис
    // (фракційне запрошення, ТЗ-4 R-C4.1). Ключ непрозорий (хеш ключа
    // персонажа) -- сервер розгортає його сам серед друзів відправника.
    // Себе не віддаємо з тієї ж причини, що й PickedName().
    string PickedKey()
    {
        OZ_ContactEntry e = Picked();
        if (!e || e.Me)
