...
class OZ_PdaInput
{
    ...
    static void Poll()
    {
        ...
        // «КПК БЕЗ ПРЕДМЕТА» (D132) -- єдиний випадок, коли клавіша відкриває.
        //
        // Правило власника лишається: прилад у руках відкриває дія, і тільки
        // вона. Але віртуальному приладу дії ніде взятись -- предмета немає,
        // -- і без цієї гілки VirtualDevice.Enabled не давав гравцеві нічого.
        // Клієнт тут не вирішує, чи йому положено: питає сервер, і той або
        // каже "pda_virtual", або мовчить. Прилад на грудях чи в руках
        // означає, що є що застосувати, і клавіша знову лише закриває.
        if (!OZ_PdaHud.Device())
            OZ_Rpc.Request(OZ_PdaConst.PAGE_DEVICE, OZ_PdaConst.OP_VIRTUAL_OPEN, "{}");
    }
}
